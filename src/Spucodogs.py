...
class SpuCoDogsDataset(Dataset):
        ...
        if self.calculate_gradcam:
            ...
            with tqdm(
                ...
                for idx in range(total_images):
                    img_path = os.path.join(self.img_dir, self.df.iloc[idx]["img_path"])
                    image = Image.open(img_path).convert("RGB")
                    cam = self.clip_gradcam.calculate_gradcam(image, self.prompts)
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(img_path).convert("RGB")
        label = torch.tensor(row["subclass"], dtype=torch.long)
...
def prepare_spucodogs_metadata(root_dir):
    dogs_dir = os.path.join(root_dir, "spuco_dogs")
    data = []

    for split in ["train", "val", "test"]:
        split_id = 0 if split == "train" else (1 if split == "val" else 2)

        pattern = os.path.join(dogs_dir, split, "big_dogs", "indoor", "*.jpg")
        for img_path in glob.glob(pattern):
            rel_path = os.path.relpath(img_path, root_dir)
            data.append(
                {"img_path": rel_path, "subclass": 0, "place": 0, "split": split_id}
            )

        pattern = os.path.join(dogs_dir, split, "big_dogs", "outdoor", "*.jpg")
        for img_path in glob.glob(pattern):
            rel_path = os.path.relpath(img_path, root_dir)
            data.append(
                {"img_path": rel_path, "subclass": 0, "place": 1, "split": split_id}
            )

        pattern = os.path.join(dogs_dir, split, "small_dogs", "indoor", "*.jpg")
        for img_path in glob.glob(pattern):
            rel_path = os.path.relpath(img_path, root_dir)
            data.append(
                {"img_path": rel_path, "subclass": 1, "place": 0, "split": split_id}
            )

        pattern = os.path.join(dogs_dir, split, "small_dogs", "outdoor", "*.jpg")
        for img_path in glob.glob(pattern):
            rel_path = os.path.relpath(img_path, root_dir)
            data.append(
                {"img_path": rel_path, "subclass": 1, "place": 1, "split": split_id}
            )
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            for i in range(bsz):
                ...
                y = int(full_df.loc[row_idx, "subclass"])
...
def main():
    ...
    cfg = {
        ...
        "num_envs": 4,
        "prompt": ["a dog"],
        "data_dir": os.path.join(PROJECT_ROOT, "data", "spucodogs", "SpuCoAnimals"),
        "beta": 1.0,
        "cls_weight": 1.0,
        "bce_weight": 1.0,
        "gcam_weight": 40.0,
    ...
    log_path = os.path.join(output_dir, "spucodogs.log")
    model_save_path = os.path.join(output_dir, "best_spucodogs.pth")
    ...
    with open(log_path, "w", encoding="utf-8") as log_f:
        ...
        df = prepare_spucodogs_metadata(root_dir)
        ...
        print(format_group_counts_line("Train", train_counts), flush=True)
        print(format_group_counts_line("Val", valid_counts), flush=True)
        print(format_group_counts_line("Test", test_counts), flush=True)

        transform = create_original_transform("resnet50")

        print("Creating datasets with GradCAM", flush=True)
        train_dataset = SpuCoDogsDataset(
            df_train,
            root_dir,
            transform=transform,
            calculate_gradcam=True,
        ...
        valid_dataset = SpuCoDogsDataset(
            df_valid,
            root_dir,
            transform=transform,
        ...
        print(format_test_group_acc_line(test_group_acc), flush=True)
